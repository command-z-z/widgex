...
impl WebContextImpl {
  ...
  pub fn create_context(context: WebContext) -> Self {
    // Widgets never navigate back/forward, so the default WEB_BROWSER cache
    // model (which keeps a large page cache) wastes memory. DOCUMENT_VIEWER
    // disables the page cache and reduces in-memory resource caches.
